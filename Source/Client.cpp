...
/************************************************************************/
    ...
    #define CHECK_NEED_AP( need_ap )                                                                                                                                                         \
        { if( IsTurnBased && !IsTurnBasedMyTurn() )                                                                                                                                          \
              break; if( Chosen->GetParam( ST_CURRENT_AP ) < 0 ) { if( IsTurnBased ) { if( Chosen->GetParam( ST_CURRENT_AP ) )                                                               \
                                                                                                           AddMess( FOMB_GAME, FmtCombatText( STR_COMBAT_NEED_AP, need_ap ) ); break; } else \
																																												   return; } \
        }
    #define CHECK_NEED_REAL_AP( need_ap )                                                                                                                                                    \
        { if( IsTurnBased && !IsTurnBasedMyTurn() )                                                                                                                                          \
              break; if( Chosen->GetRealAp() < 0 ) { if( IsTurnBased ) { if( Chosen->GetRealAp() )                                                                                           \
                                                                                          AddMess( FOMB_GAME, FmtCombatText( STR_COMBAT_NEED_AP, ( need_ap ) / AP_DIVIDER ) ); break; } else \
                                                                                                                                                                                 return; } }
    #endif // FORP_ENGINE
...
void FOClient::SScriptFunc::Global_DrawMapSprite( ushort hx, ushort hy, ushort proto_id, uint spr_id, int spr_index, int ox, int oy )
{
    ...
    if( !no_light )
        spr.SetLight( proto_item ? proto_item->Corner : CORNER_EAST_WEST, Self->HexMngr.GetLightHex( 0, 0 ), Self->HexMngr.GetMaxHexX(), Self->HexMngr.GetMaxHexY() );
        ...
        for( int i = 0; i < ( GameOpt.MapHexagonal ? 6 : 4 ); i++ )
        {
            ...
            for( uint j = 0, jj = ( GameOpt.MapHexagonal ? dist : dist * 2 ); j < jj; j++ )
            {
                ...
                if( FLAG( GameOpt.LookChecks, LOOK_CHECK_LOOK_DATA ) )
                {
                    ...
                    if( newdist == 0 )
                        newdist = 1;
                    FOClient::Self->HexMngr.TraceBullet( base_hx, base_hy, hx_, hy_, newdist, 0.0f, NULL, false, NULL, 0, NULL, &block, NULL, true );
                    hx_ = block.first;
                    hy_ = block.second;
                ...
                else
                {
                    if( FLAG( GameOpt.LookChecks, LOOK_CHECK_DIR ) )
                    {
                        ...
                        UShortPair block;
                        FOClient::Self->HexMngr.TraceBullet( base_hx, base_hy, hx_, hy_, dist_, 0.0f, NULL, false, NULL, 0, NULL, &block, NULL, false );
                        hx_ = block.first;
                        hy_ = block.second;
                    }
                    ...
                    {
                        UShortPair block;
                        FOClient::Self->HexMngr.TraceBullet( base_hx, base_hy, hx_, hy_, 0, 0.0f, NULL, false, NULL, 0, NULL, &block, NULL, true );
                        hx_ = block.first;
                        hy_ = block.second;
                    }
                }
        ...
        if( FLAG( GameOpt.LookChecks, LOOK_CHECK_LOOK_DATA ) )
        {
            for( int i = 0; i < ( GameOpt.MapHexagonal ? 6 : 4 ); i++ )
            {
                ...
                for( uint j = 0, jj = ( GameOpt.MapHexagonal ? dist_hear : dist_hear * 2 ); j < jj; j++ )
                {
                    ...
                    if( newdist == 0 )
                        newdist = 1;
                    FOClient::Self->HexMngr.TraceBullet( base_hx, base_hy, hx_, hy_, newdist, 0.0f, NULL, false, NULL, 0, NULL, &block, NULL, false );
                    hx_ = block.first;
                    hy_ = block.second;
